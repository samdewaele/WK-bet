...
import { emailGroupStageComplete, emailRoundComplete, emailRoundReminder } from "@/lib/email";

const ROUNDS_IN_ORDER = ["Group", "R32", "R16", "QF", "SF", "3rd", "Final"] as const;
type Round = (typeof ROUNDS_IN_ORDER)[number];
// Remind for all rounds (including Group stage).
const REMINDER_ROUNDS = ["Group", "R32", "R16", "QF", "SF", "3rd", "Final"] as const;
...
export async function buildRoomLeaderboard(roomId: string): Promise<LeaderboardRow[]> {
...
/** Send a reminder to all active members 24h before any round's first match. */
  ...
  for (const round of REMINDER_ROUNDS) {
    ...
    for (const room of rooms) {
      for (const member of room.members) {
        const { email, name } = member.user;
        if (!email) continue;
        emailRoundReminder(email, name ?? "there", round, room.name, room.id, deadline).catch(() => {});
        sent++;
      }
    }
